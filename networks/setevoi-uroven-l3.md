...
1. Отправляет **Neighbor Solicitation (NS)(Догомательство до соседа)** на свой будущий LLA.
...
Это специальный тип multicast-адреса, используемый для **Neighbor Discovery Protocol (NDP)** — аналога ARP в IPv4. Его главная задача — эффективно находить MAC-адрес соседнего устройства по известному IPv6-адресу.
...
1. Устройство хочет найти MAC соседа (например, для `2001:db8::1a2b:3c4d:5e6f`).
2. Вычисляет Solicited-Node адрес: `ff02::1:ff5e:6f`.
3. Отправляет **Neighbor Solicitation (NS)** на этот multicast.
4. **Целевое устройство** (у которого IPv6 заканчивается на `...:5e6f`) отвечает **Neighbor Advertisement (NA)** со своим MAC.
...
```
+-------------------+       +-------------------+       +-------------------+  
|    Устройство A   |       |    Маршрутизатор  |       |    Устройство B   |  
| (2001:db8::1)     |       | (2001:db8::ff)    |       | (2001:db8::2)     |  
| MAC: AA:BB:CC:DD:EE |       | MAC: RR:RR:RR:RR:RR |       | MAC: BB:BB:BB:BB:BB |  
+-------------------+       +-------------------+       +-------------------+  
          |                          |                          |  
          |                          |                          |  
          | 1. Router Solicitation (RS) → ff02::2               |  
          |--------------------------->|                         |  
          |                          |                          |  
          | 2. Router Advertisement (RA) ← ff02::1              |  
          |<---------------------------|                         |  
          | (Префикс: 2001:db8::/64, шлюз: 2001:db8::ff)        |  
          |                          |                          |  
          | 3. Neighbor Solicitation (NS) → ff02::1:ff00:2      |  
          |--------------------------------------------------->|  
          | (Для поиска MAC устройства B)                       |  
          |                          |                          |  
          | 4. Neighbor Advertisement (NA) ← BB:BB:BB:BB:BB     |  
          |<---------------------------------------------------|  
          | (MAC устройства B)                                  |  
          |                          |                          |  
          | 5. DAD (NS на свой адрес fe80::... → ff02::1:ffXX:XXXX) |  
          | (Проверка уникальности своего LLA)                  |  
