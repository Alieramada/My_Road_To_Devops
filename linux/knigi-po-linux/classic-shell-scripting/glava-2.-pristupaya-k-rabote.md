...
Как правило, вы устанавливаете LC\_ALL для принудительного использования одного языкового стандарта, а LANG - для обеспечения резервного языкового стандарта. В большинстве случаев вам следует избегать установки каких-либо других переменных LC\_xxx. Например, хотя может показаться более точным задать значение LC\_COLLATE при использовании команды sort, это значение может конфликтовать с параметром типа LC\_ или полностью игнорироваться, если задано значение LC\_ALL.

Стандарты ISO C и C++ предписывают только одно стандартное языковое имя - C: оно выбирает традиционное поведение, ориентированное на ASCII. В POSIX указано одно дополнительное языковое имя - POSIX, которое эквивалентно C.

За исключением названий C и POSIX, языковые стандарты не стандартизированы. Однако большинство поставщиков придерживаются схожих, но не идентичных соглашений об именовании. Название языкового стандарта кодирует язык, территорию и, при необходимости, набор кодов и модификатор. Обычно он представлен строчным двухбуквенным кодом языка по стандарту ISO 639,\* символом подчеркивания и двухбуквенным кодом страны по стандарту ISO 3166-1 в верхнем регистре,† необязательно с точкой и кодировкой набора символов, а также знаком at и словом-модификатором. Иногда также используются названия языков. Вы можете перечислить все распознанные локальные названия в вашей системе следующим образом:
...
Вы можете запросить сведения о конкретной языковой переменной, определив языковой стандарт в среде (здесь в качестве префикса к команде) и выполнив команду locale с параметром –ck и переменной LC\_xxx. Вот пример из системы Sun Solaris, которая предоставляет информацию о датском времени:

```bash
$ LC_ALL=da locale -ck LC_TIME                 Get locale information for Danish time
 LC_TIME
d_t_fmt="%a %d %b %Y %T %Z"
d_fmt="%d-%m-%y"
t_fmt="%T"
...
Количество доступных языков варьируется в широких пределах. В ходе исследования около 20 версий Unix было обнаружено, что в системах BSD их вообще нет (в них отсутствует команда locale), в некоторых системах их всего пять , а в последних версиях GNU/Linux - почти 500. Поддержка языковых стандартов может устанавливаться на усмотрение системного менеджера, поэтому даже одна и та же версия операционной системы на двух одинаковых компьютерах может иметь разную поддержку языковых стандартов. Мы обнаружили, что требования к файловой системе для поддержки языковых стандартов в некоторых системах приближаются к 300 МБ\*\*.

\*

†

\*\* МБ = мегабайт, приблизительно 1 миллион байт, где один байт в настоящее время обычно равен восьми битам (двоичным цифрам), хотя в прошлом использовались байты как большего, так и меньшего размера. Несмотря на префикс metric, при использовании компьютера M обычно означает $$2^{20}=1,048,576.$$ Удобное эмпирическое правило гласит, что один мегабайт равен объему текста в книге (300 <kbd>страниц</kbd> × 60 строк на страницу × 60 символов/строка = 1 080 000 символов).

Несколько пакетов GNU были интернационализированы, и для многих языков была добавлена поддержка локализации. Например, для итальянского языка GNU ls предлагает следующую справку :

```bash
LC_ALL=it_IT ls --help Get help for GNU ls in Italian
Uso: ls [OPZIONE]... [FILE]...
...
Обратите внимание, что если перевод недоступен (пятая строка вывода), то используется язык оригинала, английский. Названия программ и опции не переводятся, поскольку это нарушило бы переносимость программного обеспечения.

В настоящее время в большинстве систем отсутствует поддержка, позволяющая программисту shell решать проблемы интернационализации и локализации. Однако на сценарии shell часто влияют региональные стандарты, в частности, порядок сопоставления и диапазоны символов, выраженных в квадратных скобках , в регулярных выражениях. Хотя мы описываем классы символов, сопоставление символов и классы эквивалентности в разделе “Что такое регулярное выражение?” \[3.2.1], в большинстве систем Unix, по-видимому, довольно сложно определить из документации по языковому стандарту или инструментальных средств, какие именно символы являются элементами символа и эквивалентности классы и доступные символы для сопоставления. Это отражает незрелость поддержки языковых стандартов в современных системах.

Когда установлен пакет GNU gettext\*, его можно использовать для поддержки интернационализации и локализации сценариев командной оболочки. Это сложная тема, которую мы не рассматриваем в этой книге, но подробности вы можете найти в разделе Подготовка командной оболочки Раздел скриптов для интернационализации в руководстве gettext.

Из-за широких различий в локальной поддержке и отсутствия стандартизированных названий языковых стандартов в сценариях переносимой оболочки сложно что-либо делать с языковыми стандартами, кроме как принудительно использовать традиционную локаль, установив для LC\_ALL значение C. Мы делаем это в некоторых сценариях, описанных в этой книге, когда зависимость от языка в противном случае могла бы привести к неожиданным результатам.

### 2.9 Краткое описание

Выбор языка компиляции по сравнению с языком написания сценариев обычно делается в зависимости от потребностей приложения. Языки сценариев обычно работают на более высоком уровне, чем компилируемые языки, и потеря производительности часто с лихвой компенсируется скоростью, с которой может осуществляться разработка, и возможностью работать на более высоком уровне.

Shell является одним из наиболее важных и широко используемых скриптовых языков в мире. Среда Unix. Поскольку он широко распространен, а также благодаря стандарту POSIX, можно писать программы-оболочки, которые будут работать на платформах самых разных производителей. Поскольку shell функционирует на высоком уровне, программы shell очень эффективны; вы можете многое сделать с относительно небольшими затратами труда.

Первая строка #! должна использоваться для всех shell-скриптов; этот механизм обеспечивает вам гибкость и возможность писать скрипты на выбранном вами shell или другом языке.

Shell является полноценным языком программирования. До сих пор мы рассматривали основы работы с командами, параметрами, аргументами и переменными, а также базовые функции вывода с помощью echo и printf. Мы также рассмотрели основные операторы перенаправления ввода-вывода <, >, >> и /, с которыми, как мы надеемся, вы уже знакомы.

Командная оболочка ищет команды в каждом каталоге, указанном в $PATH. Обычно у вас есть личный каталог bin, в котором вы храните свои личные программы и скрипты, и вы можете указать его в PATH, выполнив назначение в вашем файле .profile.

Мы рассмотрели основы доступа к аргументам командной строки и простой трассировки выполнения.

Наконец, мы обсудили интернационализацию и локализацию - темы, важность которых возрастает по мере того, как компьютерные системы адаптируются к вычислительным потребностям все большего числа людей в мире. Хотя поддержка сценариев shell в этой области по-прежнему ограничена, программисты shell должны знать о влиянии языков на их код.
