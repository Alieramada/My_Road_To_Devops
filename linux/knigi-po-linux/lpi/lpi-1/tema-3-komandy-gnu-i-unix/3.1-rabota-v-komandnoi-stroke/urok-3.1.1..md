...
## Использование истории ваших команд <a href="#using_your_command_history" id="using_your_command_history"></a>

Вы часто тщательно изучаете правильное использование команды и успешно запускаете её со сложным набором опций и аргументов. Но что происходит через несколько недель, когда вам нужно запустить ту же команду с теми же опциями и аргументами, но вы не можете вспомнить детали? Вместо того чтобы начинать поиск с нуля, вы часто можете восстановить исходную команду с помощью `history`.

При вводе `history` будут показаны самые последние команды, которые вы выполнили, причём самая последняя из них будет показана последней. Вы можете легко выполнить поиск по этим командам, передав определённую строку в команду `grep`: В этом примере будет выполнен поиск любой команды, содержащей текст `bash_history`:\
...
Здесь возвращается одна команда вместе с номером последовательности 1605.

Если говорить о `bash_history`, то это на самом деле название скрытого файла, который вы должны найти в домашнем каталоге вашего пользователя. Поскольку это скрытый файл (обозначается точкой перед именем), он будет виден только при просмотре содержимого каталога с помощью `ls` с аргументом `-a`:
...
Что находится в файле `.bash_history`? Посмотрите сами: вы увидите сотни и сотни ваших последних команд. Однако вы можете быть удивлены, обнаружив, что некоторые из ваших _последних_ команд отсутствуют. Это связано с тем, что, хотя они мгновенно добавляются в динамическую базу данных `history`, последние изменения в истории команд не записываются в файл `.bash_history` до завершения сеанса.

Вы можете использовать содержимое `history` для ускорения и повышения эффективности работы в командной строке с помощью клавиш со стрелками вверх и вниз на клавиатуре. При многократном нажатии клавиши со стрелкой вверх в командной строке будут отображаться последние команды. Когда вы дойдёте до команды, которую хотите выполнить во второй раз, вы можете запустить её, нажав Enter. Это позволяет легко вспоминать и при необходимости изменять команды несколько раз во время сеанса работы в командной строке.

## Упражнения с руководством <a href="#sec.103.1_01-ge" id="sec.103.1_01-ge"></a>

1. Используйте  `man` для определения того, как сообщить `apropos` о необходимости вывода краткой команды, чтобы она выводила только краткое сообщение об использовании, а затем завершала работу.
2. Используйте систему `man` для определения того, какая лицензия на авторские права назначена для команды `grep`

## Исследовательские упражнения <a href="#sec.103.1_01-ee" id="sec.103.1_01-ee"></a>
...
2. Выведите на печать последние двадцать строк динамической базы данных `history` и файла `.bash_history` для их сравнения.
3. С помощью инструмента `apropos` найдите на странице `man` команду, которая потребуется для отображения размера подключённого физического блочного устройства в байтах, а не в мегабайтах или гигабайтах.

## Краткие сведения <a href="#sec.103.1_01-su" id="sec.103.1_01-su"></a>

На этом уроке вы узнали:

* Как получить информацию о местоположении вашей файловой системы и программном стеке операционной системы.
* Как найти справку по использованию команды.
* Как определить расположение файловой системы и тип двоичных файлов команд.
* Как находить и повторно использовать ранее выполненные команды.

В этом уроке обсуждались следующие команды:

`pwd`

Вывеcти путь к текущему рабочему каталогу.

`uname`

Вывести на печать аппаратную архитектуру вашей системы, версию ядра Linux, дистрибутив и версию дистрибутива.

`man`

Получить доступ к файлам справки, документирующим использование команд.

`type`

Вывести местоположение файловой системы и ввести ее для одной или нескольких команд.

`which`

Вывести расположение файловой системы для команды.

`history`

Распечатать или повторно использовать команды, которые вы ранее выполнили.

## Ответы на упражнения с руководством <a href="#sec.103.1_01-age" id="sec.103.1_01-age"></a>

1.  Используйте `man` для определения того, как сообщить `apropos` о необходимости вывода краткой команды, чтобы она выводила только краткое сообщение об использовании, а затем завершала работу.

    Запустите `man apropos` и прокрутите вниз раздел «Параметры», пока не дойдёте до абзаца с `--usage`
2.  Используйте систему `man` для определения того, какая лицензия на авторские права назначена для команды `grep`

    Запустите `man grep` и прокрутите документ вниз до раздела «Авторские права». Обратите внимание, что программа использует авторские права Фонда свободного программного обеспечения(Free Software Foundation.).

## &#x20;Ответы на исследовательские упражнения <a href="#sec.103.1_01-aee" id="sec.103.1_01-aee"></a>

1.  Определите аппаратную архитектуру и версию ядра Linux, используемую на вашем компьютере, в удобном для чтения формате вывода.

    Запустите `man uname`, прочтите раздел «Описание» и определите аргументы команды, которые будут отображать только те результаты, которые вам нужны. Обратите внимание, что `-v` покажет версию ядра, а `-i` — аппаратную платформу.
...
3.  С помощью инструмента `apropos` найдите на странице `man` команду, которая потребуется для отображения размера подключённого физического блочного устройства в байтах, а не в мегабайтах или гигабайтах.
    ...
    Один из способов — запустить `apropos` со строкой `block`, просмотреть результаты, отметить, что `lsblk` перечисляет блочные устройства (и, следовательно, является наиболее подходящим инструментом для наших нужд), запустить `man lsblk`, прокрутить раздел «Описание» и отметить, что `-b` отображает размер устройства в байтах. Наконец, запустите `lsblk -b` и посмотрите, что получится.
