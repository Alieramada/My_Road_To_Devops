...
2. Напишите программу на Python с использованием модуля re, которая проверит что исходная строка содержит только буквы или числа

```python
import re

# Функция для проверки, состоит ли строка только из букв или чисел
def is_only_letters_or_numbers(input_string):
    # Регулярное выражение для проверки наличия только букв или цифр
    pattern = r'^[a-zA-Z0-9]*$'
...
# Проверка строки
if is_only_letters_or_numbers(user_input):
    print("Строка содержит только буквы или цифры.")
else:
    print("В строке присутствуют символы, отличные от букв и цифр.")
```

3. Какой модуль Pyhon поддерживиает регулярные выражения?
...
4. Чтое делает функция re.match

* a) Находит соотвествие шаблону с начала строки
* b) Находит все вхождения в строку соответствующие шаблону
* c) Такая функция не существует
* d) Ничего из выше перечисленного

Ответ a)

5. Какой ожидается вывод

```
import re s="we are humans" 
...
Регулярные выражения: [https://docs.python.org/3.2/library/re.html](https://docs.python.org/3.2/library/re.html)

Документация по Textwrap[https://docs.python.org/3/library/textwrap.html](https://docs.python.org/3/library/textwrap.html)

Документация по Unicode: [https://docs.python.org/3/howto/unicode.html](https://docs.python.org/3/howto/unicode.html)
