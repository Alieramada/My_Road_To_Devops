...
7. В чем разница между sort и sorted?

Методы sort и sorted в Python выполняют одну и ту же функцию — сортировку элементов, но отличаются тем, как они применяются:\\

**Sorted** — это встроенная функция в Python, которая принимает итерируемый объект (например, список, кортеж, строку) и возвращает отсортированный список. Пример использования:

```python
numbers = [3, 1, 4, 2]
sorted_numbers = sorted(numbers)
print(sorted_numbers)  # Вывод: [1, 2, 3, 4]
```

Функция sorted() сохраняет исходный объект неизменным, возвращая новый отсортированный список.

**Sort** — это метод, который можно применять к спискам, строкам и другим изменяемым итерируемым объектам для сортировки их элементов на месте. Пример:

```python
fruits = ['apple', 'banana', 'cherry']
fruits.sort()
print(fruits)  # Вывод: ['apple', 'banana', 'cherry'] (отсортировано по алфавиту)
```

Метод sort() изменяет исходный объект, сортируя его элементы.

## Читать далее&#x20;

• Изучите основные команды Linux:[https://maker.pro/linux/tutorial/basic-linux-commands-for-beginners](https://maker.pro/linux/tutorial/basic-linux-commands-for-beginners)\
• Документация по Selenium webdriver:  [https://selenium-python.readthedocs.io/index.html](https://selenium-python.readthedocs.io/index.html)
